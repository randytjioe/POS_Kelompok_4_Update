...
const productController = {
  ...
  addProduct: async (req, res) => {
    try {
      const { name, stock, harga, brand_id, gender } = req.body;
      const image_url = process.env.render_image + req.file.filename;
      const stock_update = 0;
      const data = {
        name,
        stock,
        harga,
  ...
  updateProduct: async (req, res) => {
    try {
      const { id, name, stock, harga, brand_id, gender } = req.body;
      const data = { name, stock, harga, brand_id, gender };
